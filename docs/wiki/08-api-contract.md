...
- `providers` 包含全部注册 Provider，`enabled` 标记展示；新增 Provider 时由 `fetcher.GetAll()` 动态返回
...
- `TestConnection` 的 platform 字符串 = 注册表 id（"kimi"/"xfyun"/"opencode-go"/"mimo"/"deepseek"/"glm"/"openrouter"/"aliyun"/"bailian"/"new-api"），是前后端契约
